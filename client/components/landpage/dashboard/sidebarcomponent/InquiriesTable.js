...
// Dynamically import DataTables components on client side only
const DataTable = dynamic(() => import("datatables.net-react"), { ssr: false });
const DataTablesCore = dynamic(() => import("datatables.net-bs5"), {
  ssr: false,
});

// Load jQuery globally on the client
if (typeof window !== "undefined") {
  require("jquery");
  // You can also attach jQuery to window if needed by plugins
  window.jQuery = window.$ = require("jquery");
}

// DataTable.use(DataTablesCore);
if (typeof window !== "undefined" && DataTable && DataTablesCore) {
  DataTable.use(DataTablesCore);
}
